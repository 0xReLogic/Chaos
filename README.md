...
graph LR
    subgraph Bell State Circuit
        q0_in(|0⟩) -- H --> q0_c(●) --o q0_out
        q1_in(|0⟩) -- I --> q1_c(⊕) --o q1_out
        q0_c --- q1_c
    end
```
...
```mermaid
graph LR
    subgraph GHZ State Circuit
        q0_in(|0⟩) -- H --> q0_c1(●) --o q0_c2(●) --o q0_out
        q1_in(|0⟩) -- I --> q1_c1(⊕) --o q1_c2(I) --o q1_out
        q2_in(|0⟩) -- I --> q2_c1(I) --o q2_c2(⊕) --o q2_out
        q0_c1 --- q1_c1
        q0_c2 --- q2_c2
    end
```
...
graph TD
    subgraph QFT
        q0_in[|x₀⟩] --> H0[H] --> CROT1[C-R₂] --> CROT2[C-R₃] --> q0_mid
        q1_in[|x₁⟩] --> H1[H] --> CROT3[C-R₂] --> q1_mid
        q2_in[|x₂⟩] --> H2[H] --> q2_mid

        q1_mid -- control --- CROT1
        q2_mid -- control --- CROT2
        q2_mid -- control --- CROT3
...
```mermaid
graph LR
    subgraph Grover's Iteration
        direction LR
        subgraph Oracle
            Uf[U_f]
        end
        subgraph Amplifier
            H1[H^⊗n] --> X[X^⊗n] --> CZ[C-Z] --> H2[H^⊗n]
        end
        input([|s'⟩]) --> Oracle --> Amplifier --> output
    end
```
...
```mermaid
graph TD
    subgraph Shor's Algorithm
        direction LR
        subgraph Control Register (n qubits)
            c_in[|0⟩^⊗n] --> H[H^⊗n] --> CU[Controlled-U_a] --> IQFT[QFT⁻¹]
        end
        subgraph Target Register (m qubits)
            t_in[|1⟩] --> CU
        end
        IQFT --> M(Measure)
    end
